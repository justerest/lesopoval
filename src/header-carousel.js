...
document.addEventListener('DOMContentLoaded', function () {
  const simple_dots = document.querySelector('.js_simple_dots');
  const dot_count = simple_dots.querySelectorAll('.js_slide').length;
  const dot_container = simple_dots.querySelector('.js_dots');
  const dot_list_item = document.createElement('li');

  function handleDotEvent(e) {
    if (e.type === 'before.lory.init') {
      for (var i = 0, len = dot_count; i < len; i++) {
        const clone = dot_list_item.cloneNode();
        dot_container.appendChild(clone);
      }
      dot_container.childNodes[0].classList.add('active');
    }
    if (e.type === 'after.lory.init') {
      for (i = 0, len = dot_count; i < len; i++) {
    ...
    if (e.type === 'after.lory.slide') {
      for (i = 0, len = dot_container.childNodes.length; i < len; i++) {
